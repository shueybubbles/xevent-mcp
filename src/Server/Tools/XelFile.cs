...
public static class XelFileTools
{
        ...
        [Description("The comma-separated list of actions and field names to include. Defaults to all.")] string actionsAndFields = "")
    {
        ...
        var eventFilter = eventNames.Split(',', StringSplitOptions.RemoveEmptyEntries);
        var actionsAndFieldsFilter = actionsAndFields.Split(',', StringSplitOptions.RemoveEmptyEntries);
        ...
        if (isHttpPath)
        {
            return await GetUrlStreamAsync(filePath, urlStreamProvider!, maxEvents, byteOffset, cancellationToken);
        }
    ...
    private static async Task<string> GetUrlStreamAsync(string filePath, IUrlStreamProvider urlStreamProvider, long maxEvents, long byteOffset, CancellationToken cancellationToken)
    {
