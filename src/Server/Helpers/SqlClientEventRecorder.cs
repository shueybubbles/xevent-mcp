...
namespace Bubbles.XEvent.MCPServer.Helpers
{
    ...
    public abstract class EventRecorderBase : EventListener
    {
        ...
        public void Start()
        {
            if (!isRecording)
            {
                Trace.TraceInformation($"{RecorderTypeName}.Start [{ThreadId}] for EventSource '{EventSourceName}'");
            }
            isRecording = true;
        }
        ...
        public void Stop()
        {
            if (isRecording)
            {
                Trace.TraceInformation($"{RecorderTypeName}.Stop [{ThreadId}] for EventSource '{EventSourceName}'");
            }
            isRecording = false;
        }
